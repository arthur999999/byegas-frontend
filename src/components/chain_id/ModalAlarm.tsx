        ...
        <div className={modal? style.alarm: style.hidden}>
            <div className={style.background}>
                ...
                <div className={style.input}>
                    <input type="text" placeholder="value gas"  onChange={(e) => { setValue(e.target.value) }} value={value} /><span>{ boolean ? "" : "/10K usd"}</span>
                </div>
                <p>in Gwei?<input onChange={boolean? ()=> setBoolean(false) : ()=> setBoolean(true)} type="checkbox"  defaultChecked={boolean} /></p>
            ...
            <div className={show? style.telegram : style.hidden}>
                <div>
                    <h2>{telegram == "Telegram already registered" ? "Telegram already registered" :  `send to bot /register ${telegram}`}</h2>
